  ...
  return (
    <div style={{ maxWidth:768, margin:'0 auto', minHeight:'100vh' }}>
      ...
      <div className="profile-section" style={{ display:'flex', flexDirection:'column', alignItems:'center', textAlign:'center' }}>
        <img src="/assets/profile.jpg" alt="Satriadevs" className="profile-avatar animate-float" onContextMenu={e => e.preventDefault()} draggable={false} />
      ...
      <div style={{ maxWidth:768, margin:'0 auto', padding:'1.5rem 1rem 6rem' }}>
        <div style={{ display:'flex', justifyContent:'center', gap:'.625rem', marginBottom:'1.5rem', flexWrap:'wrap' }}>
          <button className={`nav-btn ${section === 'catalog' ? 'nav-btn-active' : 'nav-btn-ghost'}`} onClick={() => setSection('catalog')}>
            <i className="fas fa-th-large"></i> Semua
          </button>
          <button className={`nav-btn ${section === 'history' ? 'nav-btn-active-blue' : 'nav-btn-ghost'}`} onClick={() => setSection('history')}>
        ...
        {section === 'catalog' && (
          products.length === 0
            ...
            : <div style={{ display:'grid', gridTemplateColumns:'repeat(2,1fr)', gap:'1.25rem' }}>
                {products.map((p, i) => <ProductCard key={p.id} product={p} index={i} onBuy={setSelectedProduct} />)}
              </div>
        )}
        ...
        <div style={{ textAlign:'center', paddingTop:'2.5rem', marginTop:'2rem', borderTop:'1px solid #e5e7eb' }}>
          <div style={{ display:'flex', justifyContent:'center', gap:'1.25rem', marginBottom:'1.25rem' }}>
            {['fa-instagram','fa-tiktok','fa-youtube','fa-whatsapp'].map(icon => (
              <a key={icon} href="#" style={{ width:44, height:44, borderRadius:'50%', background:'white', boxShadow:'0 4px 12px rgba(0,0,0,.08)', display:'flex', alignItems:'center', justifyContent:'center', color:'#6b7280', textDecoration:'none' }}>
      ...
      <CheckoutModal product={selectedProduct} show={!!selectedProduct} onClose={() => setSelectedProduct(null)} />
    </div>
  );
}
